...
class Student:
    ...
    def stud_info(self):
        rez = [self.surname, self.name, self.group_number, self.estimations, f'   {self.average()}', ]
        return rez


# Это только для генерации студентов
# with open('student.txt', 'w', encoding='utf8') as student_file:
#     for student in range(10):
#         us_str = input('Введи ФИ и группу через пробел: ').split()
#         estimate = ' '.join([str(randint(2, 5)) for _ in range(5)])
#         out_str = '{:<10}*{:<10}*{:<10}*{:<10}'.format(*us_str, estimate)
#         student_file.write(out_str + '\n')
with open('student.txt', encoding='utf8') as student_file:
    student_list = []
    for i__line in student_file:
        stud_file_list = i__line.rstrip().split('*')
        student_list.append(
    ...
    print('{:<11}{:<11}{:<11}{:<13}{:<13}'.format('Фамилия', 'Имя', 'Группа', 'Оценки', 'Ср.балл'))
