...
# Прям очень тяжело было
# Какой тип указывать переменной cls????
def format_for_timer(cls, new_format: str, func: Callable) -> Callable:
    """
    Функция декоратор с параметрами
    Проверяет время выполнения кода
    """
...
# Какой тип указывать cls? А так же как написать что
# возврщает декоратор? он возвращает класс декорированный?
def log_methods(format_data: str):
    """Декоратор класса, в котором применяется декоратор таймер"""
    new_format = ''.join('%' + el if el.isalpha()
                         else el for el in format_data)

    def decorated_cls(cls):
        for i_methods in dir(cls):
            if i_methods.startswith('__') is False:
                cur_method = getattr(cls, i_methods)
                decorated_method = format_for_timer(
                    cls, new_format, cur_method)
